...
export class LegacyPlatform extends pulumi.ComponentResource {
  ...
  constructor(
    name: string,
    args: LegacyPlatformArgs,
    opts?: pulumi.ComponentResourceOptions,
  ) {
    super("SprocketBot:LegacyPlatform", name, {}, opts);

    this.dbCredentials = new PostgresUser(
      `${name}-db-user`,
      {
        providers: {
          postgres: args.postgresProvider,
          vault: args.vaultProvider,
        },
    ...
    this.worker = new docker.Service(
      `${name}-worker`,
      {
        ...
        taskSpec: {
            ...
            env: {
              ...
              bot_token:
                pulumi.getStack() === "main"
                  ? config.requireSecret("legacy-bot-token-emilia")
                  : config.requireSecret("legacy-bot-token"),
              connstring: pulumi
                .all([
                ...
                .apply(
                  ([dbUser, dbPass, host, db]) =>
                    `postgresql://${dbUser}:${dbPass}@${host}/${db}?sslmode=disable`,
                ),
          ...
          networks: [args.postgresNetworkId, this.network.id],
        },
      },
      { parent: this },
    );
  ...
  private buildProductionBot(name: string, args: LegacyPlatformArgs) {
    this.bot = new docker.Service(
      `${name}-bot-service-emilio`,
      {
        auth: {
          username: config.require("docker-username"),
          password: config.requireSecret("docker-access-token"),
    ...
    this.bot2 = new docker.Service(
      `${name}-bot-service-emilia`,
      {
        auth: {
          username: config.require("docker-username"),
          password: config.requireSecret("docker-access-token"),
