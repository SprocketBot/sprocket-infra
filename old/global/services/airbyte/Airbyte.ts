...
export class Airbyte extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.volumes.workspace = new docker.Volume(
      `${name}-workspace`,
      { name: "airbyte_workspace" },
      { parent: this },
    );

    this.services.init = new docker.Service(
      `${name}-init`,
      {
        name: "airbyte-init",
        taskSpec: {
          ...
          placement: {
            constraints: ["node.labels.role==airbyte"],
          },
    ...
    this.services.db = new docker.Service(
      `${name}-db`,
      {
        name: "airbyte-db",
        taskSpec: {
    ...
    this.services.worker = new docker.Service(
      `${name}-worker`,
      {
        name: "airbyte-worker",
        taskSpec: {
    ...
    this.services.bootloader = new docker.Service(
      `${name}-bootloader`,
      {
    ...
    this.services.cron = new docker.Service(
      `${name}-cron`,
      {
        name: "airbyte-cron",
        taskSpec: {
    ...
    this.services.temporal = new docker.Service(
      `${name}-temporal`,
      {
    ...
    this.services.webapp = new docker.Service(
      `${name}-webapp`,
      {
        name: "airbyte-webapp",
    ...
    this.services.server = new docker.Service(
      `${name}-server`,
      {
        name: "airbyte-server",
