...
export class DGraph extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.dgraphNet = new docker.Network(
      `${name}-net`,
      {
        driver: "overlay",
      },
    ...
    this.zeroVolume = new docker.Volume(
      `${name}-zero-vol`,
      { name: `${name}-zero-data` },
      {
        parent: this,
      ...
      `${name}-service`,
      {
        ...
        taskSpec: {
          containerSpec: {
            ...
            env: {},
            commands: [`dgraph`, "zero", `--my=${name}-zero:5080`],
            mounts: [
              {
                type: "volume",
                target: "/dgraph",
          ...
          networks: args.platformNetworkId
            ? [args.platformNetworkId, args.ingressNetworkId, this.dgraphNet.id]
