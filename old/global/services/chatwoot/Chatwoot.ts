...
export class Chatwoot extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.secretKeyBase = new random.RandomPassword(
      `${name}-secret-key`,
      {
        upper: true,
        lower: true,
    ...
    this.postgresUser = new PostgresUser(
      `${name}-db-user`,
      {
        username: "chatwoot",
    ...
    this.postgresDb = new postgres.Database(
      `${name}-db`,
      {
        name: name,
    ...
    const policyContent = readFileSync(`${__dirname}/minio-policy.txt`)
      .toString()
      .replace(/\{\{\s*bucket\s*\}\}/g, name);

    this.s3Policy = new minio.IamPolicy(
      `${name}-s3-policy`,
      {
    ...
    this.s3PolicyAttachment = new minio.IamUserPolicyAttachment(
    ...
    this.sidekiq = new docker.Service(
      `${name}-sidekiq`,
      {
        taskSpec: {
          containerSpec: {
          ...
          networks: [args.postgresNetworkId, args.networkId],
        },
      },
      { parent: this },
    );
