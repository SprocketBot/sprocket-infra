...
export class PostgresVaultProvider extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.connection = new vault.database.SecretBackendConnection(
      `${name}-backend-conn`,
      {
        ...
        postgresql: {
          ...
          connectionUrl: pulumi
            .all([args.pg.hostname, args.pg.dbName])
            .apply(
              ([h, db]) =>
                `postgresql://{{username}}:{{password}}@${h}:5432/${db}?sslmode=disable`,
            ),
    ...
    const makeRole = (roleName: string, suffix: string) =>
      new vault.database.SecretBackendRole(
        `${name}-${roleName}`,
        {
