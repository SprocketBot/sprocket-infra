...
export class PlatformVault extends pulumi.ComponentResource {
  ...
  constructor(
    name: string,
    args: PlatformVaultArgs,
    opts?: pulumi.ComponentResourceOptions,
  ) {
    super("SprocketBot:Platform:VaultSync", name, {}, opts);

    this.redisSecret = new vault.generic.Secret(
      `${name}-redis-vault`,
      {
    ...
    this.rabbitmqSecret = new vault.generic.Secret(
      `${name}-rabbitmq-vault`,
      {
    ...
    this.postgresSecret = new vault.generic.Secret(
      `${name}-postgres-vault`,
      {
    ...
    this.postgresDataScienceSecret = new vault.generic.Secret(
      `${name}-postgres-data-science-vault`,
      {
        path: `platform/data-science/${args.environment}/postgres`,
        dataJson: pulumi
          .output(args.postgresDataScience)
          .apply((r) => JSON.stringify(r)),
      },
      { parent: this, provider: args.vaultProvider },
    );

    this.minioSecret = new vault.generic.Secret(
      `${name}-minio-vault`,
      {
