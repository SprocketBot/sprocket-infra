...
import {
  SprocketService,
  SprocketServiceArgs,
} from "./microservices/SprocketService";
...
import {
  CHATWOOT_SUBDOMAIN,
  DEV_CHATWOOT_WEBSITE_TOKEN,
  HOSTNAME,
  PRODUCTION_CHATWOOT_WEBSITE_TOKEN,
...
export class Platform extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.datastore = new PlatformDatastore(
      `${name}-datastores`,
      {
    ...
    this.database = new PlatformDatabase(
      `${name}-database`,
      {
        environmentSubdomain: this.environmentSubdomain,
    ...
    this.secrets = new PlatformSecrets(
      `${name}-secrets`,
      {
    ...
    this.igUrl = buildHost(
      "image-generation",
      this.environmentSubdomain,
      HOSTNAME,
    );
    ...
    this.core = new SprocketService(
      `${name}-sprocket-core`,
      {
        ...this.buildDefaultConfiguration("core", args.configRoot),
        labels: [...coreLabels.complete],
        flags: { database: true },
        secrets: [
          {
    ...
    this.clients = {
      ...
      imageGen: new SprocketService(
        `${name}-sprocket-image-generation-frontend`,
        {
          ...this.buildDefaultConfiguration(
            "image-generation-frontend",
            args.configRoot,
          ),
          labels: [...imageGenLabels.complete],
          configFile: {
      ...
      discordBot: new SprocketService(
        `${name}-discord-bot`,
        {
    ...
    this.services = {
      notifications: new SprocketService(
      ...
      analytics: new SprocketService(
        `${name}-server-analytics-service`,
        {
          ...
          networks: [args.monitoringNetworkId],
          secrets: [
            {
      ...
      matchmaking: new SprocketService(
      ...
      replayParse: new SprocketService(
  ...
  buildDefaultConfiguration = (
    ...
    configValues: {
      transport: pulumi
        .all([this.datastore.rabbitmq.hostname, this.key.result])
        .apply(([rmqHost, key]) =>
          JSON.stringify(
            {
      ...
      celery: {
        broker:
          this.datastore.rabbitmq?.hostname.apply((h) => `amqp://${h}`) ?? "",
        backend:
          pulumi
            .all([
              this.datastore.redis?.hostname,
              this.datastore.redis?.credentials.password,
            ])
            .apply(([h, p]) => `redis://:${p}@${h}`) ?? "",
      ...
      bot: {
        prefix:
          this.environmentSubdomain === "main"
            ? "s."
            : `${this.environmentSubdomain}.`,
      },
      gql: {
        internal: this.core?.hostname
          ? this.core.hostname.apply((h) => `${h}:3001/graphql`)
          : "",
        public: this.apiUrl,
      },
      frontend: {
