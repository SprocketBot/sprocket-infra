...
export class SprocketService extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.applyConfigurationValues = (fileContent: string) => {
      return pulumi
        .output(args.configValues)
        .apply((cv) => handlebars.compile(fileContent)(cv));
    };
    ...
    this.coreConfig = new ConfigFile(
      `${name}-config`,
      {
        filepath: args.configFile.sourceFilePath,
        transformation: (x) =>
          args.configFile.transformation
            ? this.applyConfigurationValues(x).apply(
                args.configFile.transformation,
              )
            : pulumi.output(this.applyConfigurationValues(x)),
      },
      { parent: this },
    );
    ...
    if (args.flags?.database) {
      ...
      networks.push(pulumi.output(args.configValues.database.networkId));
    }
  ...
  buildConfigs(configs: AdditionalConfigInput[] | undefined) {
    ...
    return configs.map<ConfigSpec>(
      ({ sourceFilePath, destFilePath, transformation }) => {
        ...
        const config = new ConfigFile(
          `${name}-${filename}`,
          {
            ...
            transformation: (x) =>
              transformation
                ? this.applyConfigurationValues(x).apply(transformation)
                : this.applyConfigurationValues(x),
          },
          { parent: this },
        );
