...
type EloServiceArgs = SprocketServiceArgs & {
...
export class EloService extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.dgraphSecret = new docker.Secret(
      `${name}-secret`,
      {
        name: `${name}-creds`,
    ...
    this.service = new SprocketService(
      `${name}-sprocketservice`,
      {
        ...args,
        env: {
          ...args.env,
