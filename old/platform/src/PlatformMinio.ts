...
export class PlatformMinio extends pulumi.ComponentResource {
  ...
  constructor(
    name: string,
    args: PlatformMinioArgs,
    opts?: pulumi.ComponentResourceOptions,
  ) {
    super("SprocketBot:Platform:Minio", name, {}, opts);

    this.bucket = new minio.S3Bucket(
      `${name}-bucket`,
      {
    ...
    this.imageGenBucket = new minio.S3Bucket(
      `${name}-ig-bucket`,
      {
    ...
    this.replayBucket = new minio.S3Bucket(
      `${name}-replay-bucket`,
      {
    ...
    this.minioUser = new minio.IamUser(
      `${name}-s3-user`,
      {
    ...
    const userPolicyContent = readFileSync(
      `${__dirname}/config/minio/UserPolicy.json`,
    ).toString();
    const userPolicyTemplate = handlebars.compile(
      userPolicyContent.toString(),
      { noEscape: true },
    );

    const userPolicySectionContent = readFileSync(
      `${__dirname}/config/minio/BucketSection.json`,
    ).toString();
    const userPolicySectionTemplate = handlebars.compile(
      userPolicySectionContent.toString(),
    );

    this.policy = new minio.IamPolicy(
      `${name}-s3-policy`,
      {
        policy: pulumi
          .all([
          ...
          .apply((bucketNames) =>
            userPolicyTemplate({
              content: bucketNames
                .map((b) => userPolicySectionTemplate({ bucket: b }))
                .join(","),
            }),
          ),
      },
    ...
    this.attachment = new minio.IamUserPolicyAttachment(
      `${name}-s3-policy-application`,
      {
        policyName: this.policy.name,
        userName: this.minioUser.name,
      },
      { parent: this, provider: args.minioProvider },
    );
