...
const ingressNetworkId = LayerOne.stack.requireOutput(
...
export const chatwoot = new Chatwoot("chatwoot", {
  ...
  smtp: {
    ...
    password: vault.generic
      .getSecretOutput(
        { path: "infrastructure/smtp" },
        { provider: vaultProvider },
      )
      .apply((s) => s.data["password"]) as pulumi.Output<string>,
    port: 587,
    username: "apikey",
  },
  providers: {
    vault: vaultProvider,
    postgres: postgresProvider,
    minio: new SprocketMinioProvider({
      minioHostname: minio.url,
      vaultProvider: vaultProvider,
    }),
  },
  postgres: {
