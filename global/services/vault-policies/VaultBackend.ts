...
export class VaultBackend extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: VaultBackendArgs, opts?: pulumi.ComponentResourceOptions) {
        ...
        this.policy = new vault.Policy(`${name}-policy`, {
            name: `${args.path}-policy`,
            policy: args.policyContent
        }, {
            provider: args.vaultProvider, parent: this,
        })

        this.token = new vault.Token(`${name}-token`, {
