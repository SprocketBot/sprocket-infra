...
import {ConfigFile, LogDriver, ServiceCategory, TraefikHttpLabel} from "../../utils";
...
export class Traefik extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: TraefikArgs, opts?: pulumi.ComponentResourceOptions) {
        ...
        this.staticConfig = new ConfigFile(`traefik-static-config`, {
            filepath: args.staticConfigPath,
            vars: pulumi.all([this.socketProxy.serviceName, config.get("traefik-ca-server")])
                .apply(([$serviceName, $caServer]) => ({socketProxyHostname: $serviceName, caServer: $caServer}))
        }, {parent: this})

        this.network = new docker.Network(`network`, {
            driver: "overlay",
            attachable: true,
        }, {parent: this})
        ...
        const customCa = config.get("traefik-trust-ca") ? new ConfigFile("custom-ca", {
        ...
        this.service = new docker.Service(name, {
            ...
            taskSpec: {
                containerSpec: {
                    image: "traefik:v3.0@sha256:db21af65fb9edaa04542efe69bb6ba74afa04231874b0240fcccb059547fbf24",
                    ...
                    } : null].filter(Boolean) as docker.types.input.ServiceTaskSpecContainerSpecConfig[],
                    env: {
                        LEGO_CA_CERTIFICATES: customCa ? "/usr/local/share/ca-certificates/custom-ca.crt" : ""
                    }
                },
                logDriver: LogDriver("traefik", ServiceCategory.INFRASTRUCTURE),
                placement: {
                    constraints: [
                        RoleRestriction(Role.INGRESS)
                    ].filter(Boolean),
