...
export class TimescaleRole extends pulumi.ComponentResource {
    ...
    readonly vaultRole: vault.database.SecretBackendRole | vault.database.SecretBackendStaticRole

    constructor(name: string, args: TimescaleRoleArgs, opts?: pulumi.ComponentResourceOptions) {
        super(buildUrn(URN_TYPE.LogicalGroup, `TimescaleRole:${name}`), name, {}, opts);

        this.pgRole = new postgres.Role("pg-role", {
            name: args.name,
            login: args.static
        }, {parent: this});

        if (args.static) {
            this.vaultRole = new vault.database.SecretBackendRole("vault-role", {
            ...
            this.vaultRole = new vault.database.SecretBackendStaticRole("vault-static-role", {
                dbName: args.vaultConnName,
                backend: Backend.db,
