...
export class TimescaleDb extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.network = new docker.Network(
      "timescaledb",
      { attachable: true, driver: "overlay" },
      { parent: this },
    );
    ...
    this.service = new docker.Service(
      "timescaledb",
      {
        taskSpec: {
          containerSpec: {
            image:
              "timescale/timescaledb:latest-pg15@sha256:d3951da701191387caab2285a61c2828ae0129ce7b45518add878ccdf411be74",
            // Postgres User
            user: "70",
            groups: ["70"],
    ...
    const vaultHookup = new TimescaleVault(
      "vault",
      {
    ...
    this.provider = new postgres.Provider(
      "provider",
      {
        username: this.rootAcct.username,
        password: this.rootAcct.password,
