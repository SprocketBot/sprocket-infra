...
export class TimescaleVault extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: TimescaleVaultArgs, opts?: pulumi.ComponentResourceOptions) {
        super(buildUrn(URN_TYPE.Configuration, "TimescaleVault"), name, {}, opts)
            const connString = pulumi.all([args.host, args.username, args.password, args.port, args.sslmode])
        ...
        connString.apply(console.log)
            ...
            this.connection = new vault.database.SecretBackendConnection("connection", {
                backend: Backend.db,
                name: "Sprocket-Timescale",
                postgresql: {
                    connectionUrl: connString,
                    usernameTemplate: "{{ .DisplayName }}__{{ .RoleName }}__{{ .Timestamp \"2006-January-02-15-23\" }}",
                },
            }, {parent: this});
