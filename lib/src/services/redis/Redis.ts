...
import {ConfigFile, TraefikHttpLabel} from "../../utils";
...
export class Redis extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: RedisArgs, opts?: pulumi.ComponentResourceOptions) {
        ...
        this.service = new docker.Service(`${name}-service-${pulumi.getStack()}`, {
            taskSpec: {
                containerSpec: {
                    // https://redis.io/docs/about/about-stack/
                    image: args.exposeInsights ? "redis/redis-stack:7.2.0-v2" : "redis/redis-stack-server:7.2.0-v2",
                    configs: [
                        args.config ? {
                    ...
                    ].filter(Boolean) as docker.types.input.ServiceTaskSpecContainerSpecConfig[]
                },
                networksAdvanceds: [args.exposeInsights ? {name: args.ingressNetworkId} : null].filter(Boolean) as docker.types.input.ContainerNetworksAdvanced[]
            },
            labels: args.exposeInsights ? new TraefikHttpLabel(name)
                .targetPort(8001)
