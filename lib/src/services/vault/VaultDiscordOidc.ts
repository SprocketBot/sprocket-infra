...
export class VaultDiscordOidc extends pulumi.ComponentResource {
  constructor(
    ...
    const discordOidcAuthConfig = new vault.generic.Secret(
      "discord-oidc-config",
      {
        path: discordOidcAuth.path.apply(($path) => `auth/${$path}/config`),
        dataJson: pulumi.all([args.discord]).apply(([$discord]) =>
          JSON.stringify({
    ...
    const redirectUri = pulumi
      .all([args.endpoint, discordOidcAuth.path])
      .apply(
        ([$endpoint, $path]) =>
          `${$endpoint}/ui/vault/auth/${$path}/oidc/callback`,
      );

    redirectUri.apply(console.log);

    const discordOidcAuthRoles = {
      noAccess: new vault.generic.Secret(
        "discord-oidc-config-role-no-access",
        {
          path: discordOidcAuth.path.apply(
            ($path) => `auth/${$path}/role/no-access`,
          ),
          dataJson: redirectUri.apply(($uri) =>
            JSON.stringify({
