...
export class UserPassCredential extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: UserPassCredentialArgs, opts?: pulumi.ComponentResourceOptions) {
        super(buildUrn(URN_TYPE.Utility, "UserPassCredential"), name, {}, opts)

        this.passwordResource = new random.RandomPassword("password", {
            length: 64,
            upper: true,
            lower: true,
            number: true,
        ...
        if (!args.username) this.usernameResource = new random.RandomPet("username", {}, {parent: this})

        this.secret = new vault.kv.SecretV2(`${name}-vs`, {
            mount: args.path.mount ?? getBackend(Backend.kv2).path,
            name: args.path.name,
            dataJson: pulumi.all([this.passwordResource.result, this.usernameResource?.id ?? args.username!.toString(), args.vaultData ?? {}]).apply(([$password, $username, $data = {}]) => JSON.stringify({
        ...
        }, {parent: this, ignoreChanges: args.freeze ? ["username", "password"] : undefined })
