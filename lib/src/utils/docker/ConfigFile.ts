...
export class ConfigFile extends docker.ServiceConfig {
    constructor(name: string, {filepath,vars, ...args}: ConfigFileArgs, opts?: pulumi.CustomResourceOptions) {
        let data: string = readFileSync(filepath).toString();

        const templated = pulumi.all(vars ?? {}).apply(v => compile(data, { noEscape: true })(v))
